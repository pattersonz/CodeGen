...
{
	\\rows
	return (spot[0] == x && spot[1] == x && spot[2] == x)
...
void main()
{
	...
	while(winner == 0)
	{
		...
		int prevSel;
		confirmed = false;
		while(possibleSpot(selecting) == false)
			{
				if (selecting == 8)
				   selecting = 0;
				else
		...
		while(~confirmed)
		{
			...
			else if(_button(_1,_held,_RIGHT))
			{
				prevSel = selecting;
				if (selecting == 8)
				   selecting = 0;
		...
		if (player == 1)
		{
			drawAt(selecting,redO);
		   player = 2;
		}
		else
		{
			drawAt(selecting,redCross);
		   player = 1;
		 }  
		winner = getWinner();
	}
